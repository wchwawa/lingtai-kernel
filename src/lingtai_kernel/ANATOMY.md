...
- `tool_executor.py` — `ToolExecutor`. Synchronous/parallel tool dispatch, reasoning stripping, timing, error capture, and the privileged reserved `secondary` communication side channel: `_validate_secondary` enforces the communication whitelist (send/reply/read), action policy, and recursive-key ban; `_execute_secondary` rejects excluded primary tools at runtime, then dispatches valid communication calls mechanically before the primary, bypassing LoopGuard for the secondary itself while never blocking the primary; `_execute_single` pops `secondary` before the primary LoopGuard key and reports the outcome under the top-level `_secondary` field — for `action="read"` it also forwards a bounded slice of the read payload under `_secondary.result` (capped by `SECONDARY_READ_RESULT_MAX_BYTES`); send/reply expose only a confirmation summary so handler internals (e.g. message_ids) never leak. `_execute_parallel` runs all allowed secondaries in a sequential pre-pass before worker threads to avoid concurrent human-message fan-out.
- `secondary_tools.py` — policy and schema helpers for reserved nested `secondary` calls: communication-tool whitelist (`email`/`telegram`/`wechat`/`feishu`/`whatsapp`), action whitelist (`send`/`reply`/`read` per tool), primary-tool eligibility exclusions, read-result cap constant, and the injected JSON-schema property (`secondary_tools.py:15-100`).
...
Notification previews deliberately trade completeness for latency: they let the agent notice that something arrived, not necessarily act safely from the preview alone. The operational rule lives in the substrate: answer directly only when the newest human message is complete and unambiguous; otherwise read the producer. The reserved `secondary` channel is the runtime bridge between those rules and long primary work. Eligible primary tool schemas receive a narrow nested communication-call property from `secondary_tools.py` (`secondary_tools.py:18-30`, `secondary_tools.py:66-100`): only `email`/`telegram`/`wechat`/`feishu`/`whatsapp`, only `send`/`reply`/`read`, no nested `secondary`. `ToolExecutor._validate_secondary` enforces the whitelist and recursive-key ban (`tool_executor.py:134-198`), `_execute_secondary` runs the communication call before the primary while catching all failures (`tool_executor.py:200-317`), and `_execute_single` / `_execute_parallel` attach the outcome to the primary result under `_secondary` (`tool_executor.py:400-420`, `tool_executor.py:568-609`). For `action="read"`, the executor forwards only a bounded slice under `_secondary.result`; `send`/`reply` expose a lean status summary so handler internals such as message IDs do not leak (`tool_executor.py:272-284`). This is intentionally not a general nested-workflow engine: it exists to reply promptly or fetch full message content without abandoning the primary task.
...
4. Otherwise, dispatch on agent state:
   ...
   - **ACTIVE** — after each tool-result batch, `attach_active_notifications` moves the canonical `notifications` + `_notification_guidance` payload to the latest dict-shaped tool result and strips/skeletonizes the previous holder (`base_agent/turn.py:926-937`, `meta_block.py:336-454`). If the batch has no dict result, the old holder stays live and the fingerprint remains uncommitted so IDLE delivery can retry later. **Idle-only channels** (`meta_block.IDLE_ONLY_NOTIFICATION_CHANNELS`, currently `{"post-molt"}`) are deliberately **excluded** from the ACTIVE stamp and from the fingerprint committed here (via `_fingerprint_excluding_idle_only`): they are kernel-owned continuation signals that must *wake* the agent through the IDLE/ASLEEP synthesized pair, so a `post-molt.json` written while the agent is still ACTIVE inside the `psyche.molt` tool call is left for the IDLE pass to inject rather than being silently absorbed onto the molt tool result. If post-molt were stamped + fingerprinted here, the next IDLE `_sync_notifications` would see no change and never inject the wake (the post-molt continuation bug).
