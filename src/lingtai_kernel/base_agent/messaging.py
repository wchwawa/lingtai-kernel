...
def _rerender_unread_digest(agent) -> str | None:
    ...
    publish_notification(
        ...
        instructions=(
            ...
            "a slow bash, large file write) and a preview is truncated, "
            "you may attach the read to that primary call via the "
            "reserved 'secondary' field — "
            "secondary={tool:'email', args:{action:'read', "
            "email_id:[...]}} — to pull the full body inline before "
            "the primary starts; the bounded read result is forwarded "
            "under _secondary.result on the primary tool result. "
