...
| `list`     | Progressive-disclosure index: active registry + historical `daemons/*/daemon.json` with prompt/result previews and search filters |
...
daemon/__init__.py
  ...
  ├── _handle_list/check/reclaim    — individual action handlers; list scans active entries plus historical run-dir JSON records
...
daemon/run_dir.py
  ├── DaemonRunDir.__init__         — creates folder on disk, writes daemon.json (including call_parameters) + .prompt
...
- **List is an index, not a second truth:** `daemon(action="list")` rebuilds its view from the active registry and each run directory's `daemon.json`/`.prompt`/result preview, with optional `contains`/`status`/`last` filtering. Full detail stays in `.prompt`, `result.txt`, and JSONL trace files.
